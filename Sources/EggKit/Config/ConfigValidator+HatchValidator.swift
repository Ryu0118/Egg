...
extension ConfigValidator {
    struct HatchValidator {
        ...
        func validate(_ hatch: HatchConfig) async -> [Error] {
        ...
        private func validateOutput(_ hatch: HatchConfig) -> Error? {
            hatch.output.isEmpty ? .hatchOutputEmpty : nil
        }
        
        private func validateExcludeRules(_ excludeRules: [ExcludeRule]) async -> [Error] {
            var errors: [Error] = []
        ...
        private func validateConditionalExclude(_ conditional: ConditionalExclude, context: String) async -> [Error] {
            var errors: [Error] = []
